...
function toggleCheckBoxes(formName, startsWith) {
	...
	allChecked = true;
	for(i = 0; i < len ; i++) {
		if ((checkBoxes[i].name.search(startsWith) != -1) ||
...
function changeNext(checkBox) {
	...
	for( i=0 ; i<len ; i++) {
		if (formElements[i] == checkBox) {
			if (checkBox.checked) {
				formElements[i+1].checked = true;
				formElements[i+1].disabled = true;
			}
			else {
				formElements[i+1].disabled = false;
			}
		}
	}
}
...
function syncCheckBoxes(formName, startsWith, from, to) {
	...
	len = checkBoxes.length;
	for(i = 0; i < len ; i++) {
		if ((checkBoxes[i].name.search(startsWith + from) != -1) &&
			(checkBoxes[i+1].name.search(startsWith + to) != -1)) {
			checkBoxes[i+1].checked = checkBoxes[i].checked;
