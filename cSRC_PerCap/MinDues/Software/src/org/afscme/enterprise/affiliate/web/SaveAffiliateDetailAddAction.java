...
/** 
 * @struts:action   path="/saveAffiliateDetailAdd"
 *                  name="affiliateDetailAddForm"
 *                  validate="false"
 *                  scope="request"
 *                  input="/Membership/AffiliateDetailAdd.jsp"
 */
public class SaveAffiliateDetailAddAction extends AFSCMEAction {
    
    /** Creates a new instance of SaveAffiliateDetailAddAction */
    public SaveAffiliateDetailAddAction() {
    }
    ...
    throws Exception {
        ...
        AffiliateDetailAddForm adaf = null;
        if (request.getParameter("continue") != null || request.getParameter("cancel") != null) {
            adaf = (AffiliateDetailAddForm)session.getAttribute("affiliateDetailAddForm");;                        
            cleanupSession(session);
        } else {
            adaf = (AffiliateDetailAddForm)form;
        }
        ...
        if (request.getParameter("cancel") != null) {
            request.setAttribute("affiliateDetailAddForm", adaf);
            cleanupSession(session);
            return mapping.getInputForward();
        }
        
        if (adaf.getAffPk() != null) {
            ...
            newAffil.setParentAffPk(adaf.getAffPk());
            // Retrieve cached form and set the parent fk to the value retrieve in the new form
            adaf = (AffiliateDetailAddForm)session.getAttribute("newAffiliateForm");
            adaf.setParentFk(newAffil.getParentAffPk());
            
            // cleanup the session
            cleanupSession(session);
        } else {
            // Need to find parent Affiliate based on hierarchy in Affiliate Identifier
            newAffil = adaf.getNewAffiliate();
            ...
            if (parents == null) {
                ...
                /* temporarily store NewAffiliate and AffiliateDetailAddForm in 
                 * the session for the return
                 */
                session.setAttribute("newAffiliate", newAffil);
                session.setAttribute("newAffiliateForm", adaf);
        ...
        adaf.setParentFk(newAffil.getParentAffPk()); // need for validation
        
        // validate the affiliate 
        ActionErrors errors = adaf.validate(mapping, request);
        if (!errors.isEmpty()) {
        ...
        if (request.getParameter("continue") == null) {
            ...
            ac.setAffiliateIdType(adaf.getAffIdType());
            ac.setAffiliateIdLocal(adaf.getAffIdLocal());
            ac.setAffiliateIdState(adaf.getAffIdState());
            ac.setAffiliateIdSubUnit(adaf.getAffIdSubUnit());
            ac.setAffiliateIdCouncil(adaf.getAffIdCouncil());
            ...
            if (!CollectionUtil.isEmpty(results)) {            
                ...
                session.setAttribute("affiliateDetailAddForm", adaf);                
                return mapping.findForward("SearchAffiliateFinderRedirect");
            }
        }
